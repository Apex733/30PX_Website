...
export const ServiceCarousel = ({ services }: { services: Service[] }) => {
    ...
    // Duplicate services for smooth infinite loop
    ...
    const handleMouseEnter = () => {
        ...
        if (autoScroll) {
            autoScroll.play(0.5); // Slow down to 0.5 speed
        }
    };
    ...
    const handleMouseLeave = () => {
        ...
        const autoScroll = api.plugins()?.autoScroll;
        if (autoScroll) {
            autoScroll.play(1); // Resume normal speed
        }
    };

    return (
        <div
            className="w-full mx-auto px-4 [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]"
            ...
            >
                <CarouselContent>
                    {allServices.map((service, index) => (
                        <CarouselItem key={`${service.title}-${index}`} className="md:basis-1/2 lg:basis-1/3 pl-6">
